...
var isRosUnion = '0';
var isBiznetForUser = '0';
var isSupportNPTv6 = '0';
var isSupportLte = '0';
var oteFlag = '0';
var htFlag = '0';
var IsSupportLanAsWan = '0';
var ethNum = '4';
if (IsXdProduct()) {
    ethNum = '4';
}
var supportUnnumberMode = '0';
var notSupportPON = '0';
var showWanName = '0';
...
if (1 == DoubleFreqFlag )
{
	...
	try
	{
        WlanListTotal = new Array(new stWlan("InternetGatewayDevice\x2eLANDevice\x2e1\x2eWLANConfiguration\x2e1","ath0","WirelessNet","2\x2e4GHz"),new stWlan("InternetGatewayDevice\x2eLANDevice\x2e1\x2eWLANConfiguration\x2e5","ath8","WirelessNet\x2d5G","5GHz"),null);
	}
	catch(e)
	{
...
function ControlAtmLinkConfig(Wan)
{
	...
	if((Wan.WanAccessType != 'DSL') || (!IsAdminUser() && (CfgModeWord.toUpperCase() != "DMAROCINWI2WIFI") && (CfgModeWord.toUpperCase() != "OTE")))
	{
		return;
	}
...
function ControlWANAccessType(Wan) {
	if(!WANAccessTypeRowLoaded) {
		var CommonInterfaceConfigList =  new Array(new CommonInterfaceConfig("InternetGatewayDevice.WANDevice.1.WANCommonInterfaceConfig","EPON","1000000000"),null);
		var UpIndex = 0;
...
function getMtuMaxAllowCfg(Wan)
{
    ...
    if ((specIPoEMTUMax === '') || (specIPoEMTUMax === null)) {
        specIPoEMTUMax = 1540;
    }
    ...
    var maxMtuCfg = parseInt(specIPoEMTUMax, 10);
    if (maxMtuCfg != 1500) {
        if(Wan.IPv4AddressMode.toUpperCase() == "PPPOE") {
            maxMtuCfg -= 8;
        }
...
function ControlIPv4EnableNAT()
{
    ...
    if ((curUserType != sysUserType) && (CfgModeWord.toUpperCase()  == 'TTNET2') && (ServiceList.indexOf("INTERNET") < 0)) {
        setDisable("IPv4NatType", 1);
...
function InitIPTVWANInfo()
{
	...
	var IPTVUpPortInfo = ''.toUpperCase();
	if (IPTVUpPortInfo == "")
	{
		return;
	}
...
function ControlIPv4LanWanBind()
{
	...
	if(1 == DoubleFreqFlag)
	{
		for (var i = 0; i < WlanList.length; i++)
		{
            ...
            if (IsNormalUserBrazClaroBridge() && WlanList[i].bindenable == "1") {
    ...
    if (inatelkompub == '1') {
        setDisable("IPv4BindLanList17", 0);
    }
}
...
function ControlEditMode()
{
    ...
    if ((Tdefeature == 1) || (CfgModeWord.toUpperCase() == 'TM2') ||
        (CfgModeWord.toUpperCase() == 'NOVATEL2')) {
...
function DisableUserMode(Disable)
{
    ...
    if (!IsNormalUserBrazClaroBridge()) {
...
function ControlAntel()
{
    ...
    if ((CfgModeWord.toUpperCase() == 'ANTEL'|| CfgModeWord.toUpperCase() == 'ANTEL2'|| "1" == "1")
...
function ControlPage(Wan)
{
    ...
    if (inatelkompub == '1') {
        ...
        if (((Wan.Mode == 'IP_Bridged') || (Wan.Mode== 'PPPoE_Bridged'))) {
            var optionlen = document.getElementById("WanMode").options.length;
            if (optionlen < 2) {
                $("#WanMode").append('<option id="2" value="IP_Bridged">' + "Bridge WAN" + '</option>');
                setSelect('WanMode', 'IP_Bridged');
            }
        } else {
            RemoveItemFromSelect(wanMode, 'IP_Bridged');
        }
