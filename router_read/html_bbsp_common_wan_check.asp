...
var bondingInfoArray = new Array(null);
        ...
        if ((false == isNum(Wan.IPv4MXU)) || isNaN(Wan.IPv4MXU) || parseInt(Wan.IPv4MXU,10) > maxMtuCfg || parseInt(Wan.IPv4MXU,10) < 1)
        {
            if( CfgModeWord.toUpperCase()=="DTEDATA2WIFI") {
                ...
                if (Custom_cmcc_rms == '1') {
                    ...
                    commonMTUTips = Languages["IPv4MxuAlert_common"] + specIPoEMTUMax + ".";
                    alert(commonMTUTips);
                }
                return false;
            }
        ...
        if (false == isNum(Wan.IPv4MXU) || isNaN(Wan.IPv4MXU) || parseInt(Wan.IPv4MXU,10) > maxMtuCfg || parseInt(Wan.IPv4MXU,10) < 1280)
        {
            if( CfgModeWord.toUpperCase()=="DTEDATA2WIFI"){
                ...
                if (Custom_cmcc_rms == '1') {
                    ...
                    commonMTUTips = Languages["IPv6MxuAlert_common"] + specIPoEMTUMax + ".";
                    alert(commonMTUTips);
                }
                return false;
            } else {
...
var notCheckMutilTr069Wan = "0";
function shouldCheckMutilTr069Wan() {
    if (IsXdProduct()) {
        return false;
    }

    if ((["DESKAPHRINGDU", "FIDNADESKAP", "FIDNADESKAP2"].indexOf(CfgModeWord.toUpperCase()) >= 0) || (notCheckMutilTr069Wan == '1')) {
        return false;
    }
...
{    
    ...
    if(shouldCheckMutilTr069Wan())
    {
        ...
        if ((currInstanceId != "") && (currInstanceId != Wan.domain) &&
            (addWanService.indexOf("TR069") >= 0) && (CfgModeWord.toUpperCase() != 'BRAZCLARO')) {
