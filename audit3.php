...
// DHCP relay commands
c($fp,'show ip helper-address');
c($fp,'show ip dhcp relay interface vlan111');
c($fp,'show interface vlan 111');
c($fp,'show ip interface vlan 111');
// ONU 16 current pon-onu-mng via show running
c($fp,'show running-config interface gpon-onu_1/1/1:16');
// Uplink port VLAN config
c($fp,'show interface gei_1/3/1');
// Check if any DHCP snoop on vlan 111
c($fp,'show ip dhcp snooping vlan 111');
fclose($fp);